...
def run_case(title: str, user_text: str, **state_overrides):
    ...
    attempts = out.get("routing_attempts")
...
def main():
    ...
    run_case("2) Explicit human request -> escalation", "quiero hablar con un humano")
    run_case("3) support/sale + single route mention -> direct lock", "soporte tpms, no conecta el sensor")
    run_case("4) attempts >= MAX_ROUTING_ATTEMPTS -> escalation", "ok", routing_attempts=3)

    # these should hit the LLM
    run_case("5) LLM needed: normal AA issue (should lock AA)", "Mi aire acondicionado no enfría y hace ruido.")
    run_case("6) LLM needed: ambiguous message (should ask clarifier)", "Tengo una consulta sobre un producto.")
